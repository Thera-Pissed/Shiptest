...
/obj/item/reactor_rod
	...
	var/resonance = 0
	var/rod_heat = 0
	var/depth = 1 // 1 is full in 0 is out
	var/rod_damage = 0
...
/obj/item/reactor_rod/proc/resonate(freq = 0, dampen = 0)
	accuracy = 0
	if(resonance && freq)
		accuracy = frequency_accuracy(freq, resonance) ** accuracy_exp
	activity += round((ROOT(FREQ_ROOT, freq) * accuracy * activity_multiplier) - (dampen + flat_dampen), 0.01)
	activity = max(activity, 0)
	rod_heat += freq * heat_multiplier
